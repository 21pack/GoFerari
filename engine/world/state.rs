...
mod state_tests {
    ...
    fn make_test_map() -> GameMap {
        ...
        GameMap {
            ...
            target_positions: LinkedList::new(),
            links: HashMap::new(),
            object_collidable_map: vec![false; 0],
        }
    }
    ...
    fn test_mob_with_unknown_or_none_behaviour_defaults_to_zero_speed() {
        ...
        let mob_none = state.mobs.iter().find(|m| m.tile_x == 1 && m.tile_y == 1).unwrap();
        assert_eq!(mob_none.x_speed, 0.0);
        assert_eq!(mob_none.y_speed, 0.0);

        let mob_unknown = state.mobs.iter().find(|m| m.tile_x == 2 && m.tile_y == 2).unwrap();
